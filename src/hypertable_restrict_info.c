...
typedef struct HypertableRestrictInfo
{
	...
	int			num_base_restrictions;	/* number of base restrictions
										 * successfully added */
	DimensionRestrictInfo *dimension_restriction[FLEXIBLE_ARRAY_MEMBER];	/* Sparse. key is
																			 * attrOffset of the
																			 * dimension */
...
{
	Assert(attno > 0 && AttrNumberGetAttrOffset(attno) < hri->max_attr);
	return hri->dimension_restriction[AttrNumberGetAttrOffset(attno)];
}
